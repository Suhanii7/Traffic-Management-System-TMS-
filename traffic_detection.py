...
# Ensure table exists & enable WAL mode
def setup_database():
...
def insert_vehicle_data(vehicle_classes):
    retries = 5  # Retry if locked
    for _ in range(retries):
        try:
            conn = sqlite3.connect(db_path, timeout=10)
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO vehicle_counts (car, truck, bus, motorcycle)
                VALUES (?, ?, ?, ?)
            """, (vehicle_classes["car"], vehicle_classes["truck"], vehicle_classes["bus"], vehicle_classes["motorcycle"]))
            conn.commit()
            conn.close()
            print("Vehicle data inserted into database.")
            return
...
def select_file():
    ...
    if video_source:
        lbl_status.config(text=f"Selected File: {os.path.basename(video_source)}")
...
def run_tracking():
    ...
    while cap.isOpened():  
        ...
        # Save detected data
        insert_vehicle_data(vehicle_classes)
